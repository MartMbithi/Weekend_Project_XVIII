                                    ...
                                    <div class="card-body">
                                        <table class="datatable-init table">
                                            ...
                                            <tbody>
                                                ...
                                                $store = $_GET['store'];
                                                // Combined query to fetch all required details, including subtotal calculation
$ret = "
SELECT 
    s.sale_receipt_no,
    s.sale_quantity,
    s.sale_datetime,
    s.sale_payment_amount,
    s.sale_customer_name,
    s.sale_customer_phoneno,
    p.product_code,
    p.product_name,
    SUM(((p.product_sale_price-s.sale_discount)*s.sale_quantity)) AS total
FROM 
    sales s
INNER JOIN 
    products p ON p.product_id = s.sale_product_id
INNER JOIN 
    users u ON u.user_id = s.sale_user_id 
WHERE 
    u.user_id = ? 
    AND p.product_store_id = ? 
    AND DATE(s.sale_datetime) = CURDATE()
GROUP BY 
    s.sale_receipt_no"; // Group by receipt number to get individual sales

$stmt = $mysqli->prepare($ret);
$stmt->bind_param("ss", $user_id, $store);
$stmt->execute();
$res = $stmt->get_result();

while ($sales = $res->fetch_object()) {
// Convert sale datetime to desired timezone
...
// Calculate subtotal as sum of all sale_payment_amount
...
?>
                                                ...
                                                ?>
                                                <tr >
                                                <td class="fs-30 fw-bolder">Today's Sales:</td>
                                                        <td colspan="4" class="fw-bolder"><?php echo 'Ksh.' . number_format($today_sale, 2); ?></td>
                                                    </tr>
                                            </tbody>
                                        </table>
                                    </div>
